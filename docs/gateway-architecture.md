...
## 17. Proxy mode (Track B, authoring-only) — 2026-07-11

По спеке потребителя (UAP,
`unified-agent-platform/docs/research/vpnrouter-gateway-proxy-mode-spec.md`)
добавлен `mode = "proxy"` — второй продукт с общим ядром и другим контрактом
render. Аддитивно: отсутствие `mode` = gateway, gateway-goldens байт-в-байт
неизменны (доказано `UPDATE_GOLDEN` без диффа).

Что делает proxy-режим:
- `[proxy]` → sing-box `mixed`-inbound на `listen:port` (для HTTP-CONNECT/SOCKS
  потребителей в подах), вместо TUN.
- `[subscription].strategy`: `pinned` (один outbound) или `urltest`
  (failover-группа `auto` над всем пулом, proxy-only). urltest-параметры —
  `[subscription.urltest]` с production-дефолтами.
- **Никакого `direct`/`dns`/`route.rules`/nft** — инвариант потребителя «ни
  байта в обход из RU-IP». `apply`/`rollback`/`doctor`/`explain` в proxy →
  `PROXY_MODE_NOT_APPLYABLE` (exit 2); мутаций хоста нет. Артефакт отдаётся
  через `render --out DIR` (файлы 0600, envelope только path+bytes, не контент).

Дизайн-решения (форк-аудит 3 линз + синтез поймал 3 HIGH-риска):
- Все шесть gateway-секций → `Option` (не только interfaces/routing): иначе
  `#[serde(default)]` не даёт отличить «отсутствует» от «дефолт» для конфликт-
  матрицы. Gateway-консьюмеры — через аксессоры (`tun_mtu()`, `dns_mode()`,
  `interfaces()` с post-validate `expect`).
- `assess`/`config_in_sync`/`cmd_status` **форкаются по mode ДО** доступа к
  gateway-полям — иначе паника при `interfaces=None` ломала бы инвариант
  «один JSON envelope» на `plan`/`status` у proxy-юзера.
- Дыра «gateway без interfaces/routing» (раньше parse-ошибка, теперь конфиг
  доходит до рендера) → `GATEWAY_INTERFACES_REQUIRED`/`_ROUTING_REQUIRED` +
  ранний возврат.
- Node-safety: узел не-tls/reality или reality без pbk → `Skipped{reason}`,
  не молчаливый плейнтекст-outbound.
- Cache **v2** (все outbounds для urltest; v1 pinned читается; неизвестная
  версия — громкая ошибка), оба писателя 0600.
- Dedup тегов (`-1`,`-2`) — реальные панели шлют дубли имён; route.final и
  urltest.outbounds ссылаются на deduped-теги.

**Конформность (главный риск аудита) снята deterministic-грейдером:** реальный
`sing-box check` 1.13.14 принял proxy-конфиг с domain-серверами БЕЗ dns-блока —
на fixture и на **живой ninitux-подписке** (6 vless: IP `104.194.156.93` +
домены `de/is/nl.ninitux.top`, urltest `auto`, mixed). 5 не-vless (naive + 4
hy2) корректно в skipped. 57 тестов; 3 новых golden (proxy pinned/urltest +
gateway-minimal для None→default). Файлы: [config.rs](../src/config.rs),
[render.rs](../src/render.rs) `render_proxy_sing_box`,
[subscription.rs](../src/subscription.rs) cache v2 + node-safety,
